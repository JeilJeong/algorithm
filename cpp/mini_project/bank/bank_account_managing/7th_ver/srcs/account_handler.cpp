...
ACCOUNT_PTR	AccountHandler::create_account(void)
{
	...
	this->arr[this->current_count++] = pAccount;
...
void	AccountHandler::deposit(void)
{
	...
	{
		if (this->arr[i]->get_address() == addr)
			break;
	}
	...
	cout << "	Type money : ";
	cin >> balance;
	if (this->arr[i]->add_balance(balance))
		cout << "	[REMAIN] " << this->arr[i]->get_balance() << endl;
}
...
void		AccountHandler::withdrawal(void)
{
	...
	cout << "	Type money : ";
	cin >> money;
	if (money > this->arr[i]->get_balance())
	{
		cout << "	[ERROR] remain is not enough" << endl;
		return ;
	}
	if (this->arr[i]->add_balance(-money))
		cout << "	[REMAIN] " << this->arr[i]->get_balance() << endl;
}
